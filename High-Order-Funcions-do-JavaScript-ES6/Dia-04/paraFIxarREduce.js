...
const sum = numbers.reduce((acc, valueActu) => acc + valueActu, 0)
console.log(sum);
...
const bestClass = (acc, currentValue) => (acc.grade > currentValue.grade) ? acc : currentValue;
const modifyArray = students.map((student) => { 
    return { name: student.name, courses: student.courses.reduce(bestClass).name, 
    };
 }, 0)
